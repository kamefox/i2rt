...
def _load_arm_json(name: str) -> _ArmHWConfig:
    config_dir = os.path.join(os.path.dirname(__file__), "config")
    path = os.path.join(config_dir, f"{name}.json")
    with open(path, encoding="utf-8") as f:
        raw = json.load(f)
    g = np.array(raw["gravity_comp_factor"], dtype=float)
    if g.size not in (1, 6):
        raise ValueError(f"{path}: gravity_comp_factor 须为 1 个标量或 6 个元素，当前 {g.size} 个")
    gravity: Union[float, np.ndarray] = float(g[0]) if g.size == 1 else g
    return _ArmHWConfig(
        motor_list=tuple(tuple(m) for m in raw["motor_list"]),
        directions=tuple(int(x) for x in raw["directions"]),
...
def _gravity_for_motor_robot(hw: _ArmHWConfig, with_gripper: bool) -> Union[float, np.ndarray]:
    g = np.asarray(hw.gravity_comp_factor, dtype=float).ravel()
    if g.size == 1:
        return float(g[0])
    if g.size != 6:
        raise ValueError("gravity_comp_factor 须为标量或 6 元数组")
    if with_gripper:
        return np.append(g, 1.0)
    return g.copy()
...
def get_yam_robot(
    ...
    Args:
        ...
        temp_record_flag: Passed to MotorChainRobot (motor temperature in observations).
    """
    ...
    get_robot = partial(
        ...
        gravity_comp_factor=_gravity_for_motor_robot(hw, with_gripper),
        joint_limits=joint_limits,
        kp=kp,
        kd=kd,
        zero_gravity_mode=zero_gravity_mode,
        temp_record_flag=temp_record_flag,
