...
def combine_arm_and_gripper_xml(
    ...
    if gripper_path:
        try:
            ...
            # linear_4310 / crank_4310 / teaching_handle use root body name "gripper"
            if grip_body is None:
        ...
        if grip_body is not None:
            replaced = False
            grip_name = (grip_body.get("name") or "").lower()
            merge_gripper_into_link6 = grip_name == "gripper"
            for parent in arm_root.iter():
                children = list(parent)
                for idx, child in enumerate(children):
                    if child.tag != "body" or child.get("name") not in ("link_6", "link6"):
                        continue
                    if merge_gripper_into_link6:
                        # Keep arm joint6 on link6; copy gripper geoms / child bodies under link6.
                        gi = grip_body.find("inertial")
                        if gi is not None:
                            for rm in list(child):
                                if rm.tag == "inertial":
                                    child.remove(rm)
                                    break
                            insert_at = 0
                            for j, c in enumerate(list(child)):
                                if c.tag == "joint":
                                    insert_at = j
                                    break
                            child.insert(insert_at, deepcopy(gi))
                        for sub in list(grip_body):
                            if sub.tag == "inertial":
                                continue
                            child.append(deepcopy(sub))
                    else:
                        parent.remove(child)
