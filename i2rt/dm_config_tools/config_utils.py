"""
DM Motor CAN Configuration Utilities
=====================================
Low-level CAN communication helpers for Damiao (DM) series motors.

CAN command bytes used in the DM special-message protocol:
  0x33 — Read register value from motor
  0x55 — Write register value to motor (RAM only)
  0xAA — Save current register value to motor flash (persistent)

All messages are sent to arbitration ID 0x7FF. The motor replies with a
message whose first byte is the motor ID. Register addresses and their
decode functions are defined in ``register_addr_map``.
"""
...
class RawCanInterface:
    """Raw CAN bus wrapper for DM motor special-message protocol.

    Wraps a ``can.interface.Bus`` and provides send/receive helpers used by
    the register read/write functions below. All DM special messages are sent
    to arbitration ID 0x7FF; replies are filtered by motor ID.

    Args:
        channel: CAN interface name (e.g. ``"can0"`` for socketcan,
            ``"PCAN_USBBUS1"`` for PCAN).
        bustype: python-can bus type string (``"socketcan"``, ``"pcan"``…).
        bitrate: Bus bitrate in bps (default 1 000 000).
    ...
    def _send_message_get_response(self, id: int, data: List[int], max_retry: int = 20) -> can.Message:
        """Send a CAN message and return the motor's response.

        Flushes any stale messages before sending, then retries up to
        ``max_retry`` times on CAN errors before raising.

        Args:
            id: CAN arbitration ID to send to (typically 0x7FF).
            data: 8-byte payload as a list of ints.
            max_retry: Number of send/receive attempts before giving up.

        Returns:
            The ``can.Message`` received from the motor.

        Raises:
            AssertionError: If no valid response is received after all retries.
        """
...
register_addr_map = {
    "torque_constant": (1, bytes_to_float32),
    "master_id": (7, bytes_to_uint32),
    "id": (8, bytes_to_uint32),
    "timeout": (9, bytes_to_uint32),
    "control_mode": (10, bytes_to_uint32),
    "sw_ver": (14, bytes_to_uint32),
    "gear_ratio": (20, bytes_to_float32),
    "p_max": (21, bytes_to_float32),
    "v_max": (22, bytes_to_float32),
    "torque_max": (23, bytes_to_float32),
    "vel_kp": (25, bytes_to_float32),
    "vel_ki": (26, bytes_to_float32),
    "over_voltage": (29, bytes_to_float32),
}
...
def get_special_message_response(can_interface: RawCanInterface, motor_id: int, reg_name: str) -> Any:
    ...
    # print(f"return data is {message.data}")
